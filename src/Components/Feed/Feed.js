...
export default class Feed extends Component {
    ...
    loadmore() {
        ...
        axios({
            method: 'data',
            url: 'https://www.spectrumcet.com/techonicle/getBlogs.php?bid=' + this.state.bid,
        })
            .then((res) => {
                if (res.data.blogs) {
    ...
    componentDidMount() {
        ...
        axios({
            method: 'data',
            url: 'https://www.spectrumcet.com/techonicle/trendingArticles.php',
        })
            .then((res) => {
                if (res.data.status) {
                    this.setState({
            ...
            <div style={{ backgroundColor: '#F6F6F2' }}>
                ...
                <div className="container-fluid mt-4">
                    <div className="row justify-content-center align-items-center">
                        {twoBlogArr.map(e => < TwoBlog img={image} obj={e} key={e.id} />)}

                        <div className="col-md-5 col-sm-12 content2div">
                            <div className="content2">
                                <div className="mb-3" style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <h3>Popular Articles</h3>
                                </div>
                                {popularArr.map(e => < PopularArticle img={image} obj={e} key={e.id} />)}
                            </div>
                        </div>
                    ...
                    <div className="container-fluid mb-4">
                        <div className="row justify-content-center align-items-center">
                            <div className="col-lg-8 col-md-10 p-0 py-4">
                                {feedArr.map(e =>
                                    <Article profilePic={image1} articlePic={image} obj={e} key={e.id} />
                                )}

                                <div id="addBlogs" className='pt-4' style={{ textAlign: 'center' }}>

                                    <button type="button" className="button" onClick={() => this.handleClick()}>Show more</button>
                                </div>
                            </div>
                        </div>
                    </div>
